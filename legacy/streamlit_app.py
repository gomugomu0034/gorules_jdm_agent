"""UNMAINTAINED - the original Streamlit prototype.

The web studio (backend/main.py + frontend/) has replaced this. It is kept as a
minimal reference for the LangGraph interrupt/resume protocol and for debugging
the agent without the web stack.

It does NOT work as written against the refactored agent: `welcome_node` and
`action_selection_node` were removed when the UI took over policy selection, so
the graph now expects a real first message plus the canvas on state, e.g.

    from backend.lang_graph_agent import build_graph
    graph = build_graph()
    graph.stream(
        {"messages": [HumanMessage(content="Create a refund policy...")],
         "canvas_jdm_json": ""},
        config=thread_config,
    )

Streamlit is no longer in backend/requirements.txt; install it separately to run
this (`pip install streamlit`).
"""
